...
#include <stdio.h>
#include <ctype.h>
#include <list>	 //used for implementing double linked list
#include <iostream>
#include <string> // for the string conversion in an int
#include <sstream> // used to convert into string
...
{
public:
	static void seeMenu(); // displays the menu
	static void get(int input); // receives user's input and performs the corresponding function
	static void end(); // exits the app
};

struct Time // structure for managing time of arrival and departure
{
	int min;
	int hour;
};
...
private:
	...
	Time fromWhere, toWhere;

public:
	/* --ADD/REMOVE FLIGHTS--*/
	void addFlight(); //manages the addition of flights
	static void deleteFlight(int num); //manages flight removal

	/* --DISPLAY FLIGHTS--*/
	static void displayFlightInfo(int num); //displays flight information
	static void displaySchedule();  // displays the flights that are registered in the system

	/* --BOOK SEATS--*/
	static void reserve(int num, int val); //reverve a seat in a specific plane

	/* --FLIGHT CHECKERS--*/
	static bool flightReserved(int num); // check flight is booked
	static bool alreadyReservedSeats(int num); // checks for already registerd flight in the system

	/* --GETTERS--*/
	int getFlightNo() { return this->flightNo; }
	Time getLeavingTime() { return this->fromWhere; }
	Time getArrivingTime() { return this->toWhere; }
...
private:
	...
	int CNP, tel;
	list<int> flights;
	string firstName, lastName, nationallity, address;

public:
	/* --BOOK FLIGHT-- */
	void reserved(); // book flight for each customer
	void reservedFromQueue(int num); // (call ONLY when a reservation is cancelled)
	void cancel(); //cancel a reservation

	/* --SHOW PASSENGER'S INFO-- */
	static bool displayID(int passport);

	/* --CHECHKER FOR UNIQUE PASSPORT-- */
	static bool uniqueID(int passport);

	/* --GETTERS-- */
	int getPassport() { return this->CNP; }
	string getName() { return this->firstName; }
	string getSurname() { return this->lastName; }
...
public:
	...
	static void display(int num); //displays registered customers in the queue for each flight

	Person oldest(); //returns the object of the oldest customer, through the Person class

	/* --CHECK FOR EMPTY QUEUE-- */
	bool noReservation(); // returns true if queue is empty
...
bool checkMenuOption(string s) { // checks if the input string consists ONLY of numbers
...
void  Menu::get(int selection) {

	Flight f; // class FLight's object
	...
	case 5:
		if (!plist.empty()) {
			cout << "Please insert CNP: ";
			cin >> temp;
			while (!Person::displayID(atoi(temp.c_str()))) {
				cout << "Wrong CNP!" << endl;
				cin.clear();
				cin.ignore(256, '\n');
				cin >> temp;
...
void Flight::addFlight() {
	...
	do {
		...
		// check input
		if (!checkMenuOption(temp)) {
			cout << "Please insert a valid Flight Number! " << endl;
			flag = false;
			getline(cin, temp);
		}
		else if (Flight::flightReserved(atoi(temp.c_str()))) {
...
if ((hour >= 0 && hour <= 23) && (min >= 0 && min <= 59)) {
	this->fromWhere.hour = hour;
	this->fromWhere.min = min;
	flag = true;
}
else {
	cout << "Please insert a valid boarding time (e.g. 19:40)! ";
...
if ((hour >= 0 && hour <= 23) && (min >= 0 && min <= 59)) {
	this->toWhere.hour = hour;
	this->toWhere.min = min;
	flag = true;
}
else {
...
void Flight::displayFlightInfo(int num) {
	...
	for (std::list<Flight>::iterator i = flist.begin(); i != flist.end(); ++i) {
		if (num == i->flightNo) {
			...
			convert << i->fromWhere.hour;
			convert2 << i->fromWhere.min;
			l_time = convert.str() + ":" + convert2.str();

			convert.str(std::string()); // clear stringstream "convert"
			convert2.str(std::string()); // clear stringstream "convert2"

			convert << i->toWhere.hour;
			convert2 << i->toWhere.min;
...
void Flight::reserve(int num, int val) {
	for (std::list<Flight>::iterator i = flist.begin(); i != flist.end(); ++i) {
		if (num == (i->flightNo)) {
			i->booked_seats += val;
			break;
...
bool Flight::flightReserved(int num) {
	for (std::list<Flight>::iterator i = flist.begin(); i != flist.end(); ++i) {
		if (num == i->flightNo) {
			return true;
		}
...
bool Flight::alreadyReservedSeats(int num) {
	for (std::list<Flight>::iterator i = flist.begin(); i != flist.end(); ++i) {
		if (num == (i->flightNo)) {
...
void Person::reserved() {
	...
	if (!flist.empty()) {
		...
		LOOP8:do {
			...
			if ((this->firstName.length() <= 10) && (checkString(this->firstName))) {
				flag = true;
			}
			else {
	...
	LOOP9:do {
		...
		if ((this->lastName.length() <= 10) && (checkString(this->lastName))) {
			flag = true;
		}
		else {
			cout << "Please insert a valid Name! ";
...
LOOP10:do {
	...
	if (!checkMenuOption(temp)) {
		cout << "Please insert a valid CNP" << endl;
		flag = false;
		goto LOOP10;
	}
	else if (!Person::uniqueID(atoi(temp.c_str()))) {
		cout << "Please check the validity of your CNP" << endl;
		flag = false;
		goto LOOP10;
	}
	else {
		flag = true;
		this->CNP = atoi(temp.c_str());
	}
} while (!flag);
...
this->tel = atoi(temp.c_str());
		...
		// check input
		while (!checkMenuOption(temp) && !Flight::flightReserved(atoi(temp.c_str()))) {
		...
		//DIRECT FLIGHT
		if (Flight::alreadyReservedSeats(num)) { // check for vacancy
			this->flights.push_back(num);
			Flight::reserve(num, 1);
			...
			LOOP12:cout << "\nNumber #" << counter << " : ";
				...
				if (counter > 1) { // enter if user has entered 2 or more flights
					...
					if (!(checkTime2(tLeaving, tArriving) && !Departure.compare(Destination))) {
						cout << "Please enter a flight to a valid arrival destination!" << endl;
						goto LOOP12;
					}
				}
			...
			for (std::list<int>::iterator i = nums.begin(); i != nums.end(); ++i) {
				flag = Flight::alreadyReservedSeats(*i) and flag;
...
void Person::reservedFromQueue(int num) {  //is called only from cancel()
	...
	// check if all of the customer's chosen flights have available seats
	for (std::list<int>::iterator i = queueFront.flights.begin(); i != queueFront.flights.end(); ++i) {
		flag = Flight::alreadyReservedSeats(*i) and flag;
	}

	// enter ONLY if there are availability in ALL of the customer's chosen flights
	// and book them
	if (flag) {
		Node* p = new Node;
		...
		for (std::list<int>::iterator i = queueFront.flights.begin(); i != queueFront.flights.end(); ++i) {
			...
			// delete person from the queues
			for (std::list<Queue>::iterator i2 = qlist.begin(); i2 != qlist.end(); ++i2) {
				if (*i == i2->getNo()) {
					p = i2->getFront();
					while (p != NULL) {
						if (queueFront.CNP == p->person.CNP) {
							i2->dequeue();
							Flight::reserve(*i, 1);
						}
						p = p->next;
...
void Person::cancel() {
	...
	bool isInQueue = false; //is false if the customer, that will be deleted, doesn't belong in the queue of each flight
	...
	if (!plist.empty()) {
		...
		cout << "Please give us your CNP: ";
		getline(cin, CNPid); cout << endl;

		// check input
		while (!checkMenuOption(CNPid)) {
			cout << "Please insert a valid CNP!" << endl;
			getline(cin, CNPid); cout << endl;
		}

		while (!Person::displayID(atoi(CNPid.c_str()))) {

			//exit the program if the wrong CNP is given 5 times
			if (counter == 5) {
				cout << "Wrong CNP was given too many times.";
				return;
			}
			else {
				cout << "Please check your CNP." << endl;
				getline(cin, CNPid); cout << endl;

				//check if the CNP consists only of digits
				while (!checkMenuOption(CNPid)) {
					cout << "Please insert a valid CNP!" << endl;
					getline(cin, CNPid); cout << endl;
				}
		...
		for (std::list<Queue>::iterator i = qlist.begin(); i != qlist.end(); ++i) {
			if (atoi(num.c_str()) == i->getNo()) {
				if (!i->noReservation()) {
					...
					while (p != NULL) {
						if (atoi(CNPid.c_str()) == p->person.CNP) {
							isInQueue = true;
		...
		if (!isInQueue) {
			Flight::reserve(atoi(num.c_str()), -1);

			this->reservedFromQueue(atoi(num.c_str()));
		}
...
bool Person::displayID(int passport) {
	...
	for (std::list<Person>::iterator i = plist.begin(); i != plist.end(); ++i) {
		...
		if (passport == i->CNP) {
			...
			cout << left << setw(15) << "NAME" << left << setw(15) << "SURNAME" << left << setw(15) << "CNP" << left << setw(15) << "NATIONALLITY" << left << setw(15) << "ADDRESS" << left << setw(15) << "TELEPHONE" << endl;

			cout << left << setw(15) << i->firstName << left << setw(15) << i->lastName << left << setw(15) << i->CNP << left << setw(15) << i->nationallity << left << setw(15) << i->address << left << setw(15) << i->tel << endl << endl;
			cout << "Your flight(s): ";
	...
	cout << "CNP was not found. Check again!" << endl;
	return false;
}

bool Person::uniqueID(int passport) {
	for (std::list<Person>::iterator i = plist.begin(); i != plist.end(); ++i) {
